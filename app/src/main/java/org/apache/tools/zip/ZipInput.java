 ...
 *
 */
public class ZipInput implements Closeable, Zip {
     ...
     */
    //private final RandomInputStream archive;
	private final RandomInputStream archive;
    /**
    ...
    public ZipInput(MFile file) throws IOException {
        this(file.openFileDescriptor("r"), null, true);
    }

    public ZipInput(ParcelFileDescriptor descriptor) throws IOException {
        this(descriptor, null, true);
    }

    public ZipInput(ParcelFileDescriptor descriptor, final String encoding) throws IOException {
        this(descriptor, encoding, true);
    }
     ...
     *
     * @param descriptor the archive.
     * @param encoding the encoding to use for file names, use null
     * for the platform's default encoding
     *
     * @throws IOException if an error occurs while reading the file.
     */
    public ZipInput(ParcelFileDescriptor descriptor, final String encoding, final boolean useUnicodeExtraFields) throws IOException {
        this.archiveName = descriptor.toString();
        this.encoding = encoding;
        this.zipEncoding = ZipEncodingHelper.getZipEncoding(encoding);
        this.useUnicodeExtraFields = useUnicodeExtraFields;
        //
        archive = new RandomInputStream(descriptor);
        //
     ...
     * on a null parameter
     * @param input file to close, can be null
     */
    public static void closeQuietly(final ZipInput input) {
        if (input != null) {
            try {
                input.close();
    ...
    private void positionAtCentralDirectory()
        ...
        final boolean searchedForZip64EOCD =
            archive.getPointer() > ZIP64_EOCDL_LENGTH;
        if (searchedForZip64EOCD) {
            archive.seek(archive.getPointer() - ZIP64_EOCDL_LENGTH);
            archive.readFully(WORD_BUF);
    ...
    private void skipBytes(final int count) throws IOException {
        ...
        while (totalSkipped < count) {
            final long skippedNow = archive.skip(count - totalSkipped);
            if (skippedNow <= 0) {
    ...
    private void resolveLocalFileHeaderData(final Map<ZipEntry, NameAndComment>
        ...
        for (final Iterator<ZipEntry> it = entries.iterator(); it.hasNext();) {
            ...
            while (lenToSkip > 0) {
                final long skipped = archive.skip(lenToSkip);
                if (skipped <= 0) {
