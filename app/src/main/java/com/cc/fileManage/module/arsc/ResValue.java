...
public class ResValue {
    ...
    public static ResValue parseFrom(RandomInputStream.CutStream stream) throws IOException {
        ResValue value = new ResValue();
        value.size = IUtils.readShortLow(stream);
        value.res0 = IUtils.readUInt8(stream);
        value.dataType = IUtils.readUInt8(stream);
        value.data = IUtils.readIntLow(stream);
        return value;
    }
