...
public class ResStringPoolChunk {
    ...
    public static ResStringPoolChunk parseFrom(RandomInputStream.CutStream stream) throws IOException {
        ...
        chunk.flags = IUtils.readIntLow(stream); // read flag
        chunk.stringsStart = IUtils.readIntLow(stream);
        chunk.stylesStart = IUtils.readIntLow(stream);
        ///
        ...
        for (int i = 0; i < chunk.stringCount; ++i) {
            strOffsets[i] = IUtils.readIntLow(stream);
        }
        for (int i = 0; i < chunk.styleCount; ++i) {
            styleOffsets[i] = IUtils.readIntLow(stream);
        }
        for (int i = 0; i < chunk.stringCount; ++i) {
 ...
 /*
            ...
            if (utf8) {
                ...
                int len = IUtils.readUInt8(stream);
                String str = IUtils.readString(stream, len);
                strings.add(str);
            } else {
                // The length is encoded as either one or two 16-bit integers as per the commentRef...
                //int len = (Utils.readShort(mStreamer) & 0x7f00) >> 8;
                int len = IUtils.readShortLow(stream);
                String str = IUtils.readString16(stream, len * 2);
                strings.add(str);
            }
            //
        ...
        for (int i = 0; i < chunk.styleCount; ++i) {
            ...
            stream.seek(++start);
            int len = (IUtils.readShortLow(stream) & 0x7f00) >> 8;
