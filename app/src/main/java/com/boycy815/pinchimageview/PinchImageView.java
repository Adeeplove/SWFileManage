...
public class PinchImageView extends ImageView  {
    ...
    public Matrix getInnerMatrix(Matrix matrix) {
        ...
        if (isReady()) {
            ...
            int screenRect = getWidth() * getHeight();
            int imagesRect = getDrawable().getIntrinsicWidth() * getDrawable().getIntrinsicHeight();
            //小于控件面积
            if(imagesRect < screenRect){
                int left = getWidth() / 2 - getDrawable().getIntrinsicWidth() / 2;
                int right = left + getDrawable().getIntrinsicWidth();

                int top = getHeight() / 2 - getDrawable().getIntrinsicHeight() / 2;
                int bottom = top + getDrawable().getIntrinsicHeight();
                
                tempDst = MathUtils.rectFTake(left, top, right, bottom);
            } else{
    ...
    protected float getMaxScale() {
        //控件宽/图片宽
        float w = (float)getWidth() / getDrawable().getIntrinsicWidth();
        float h = (float)getHeight() / getDrawable().getIntrinsicHeight();
    ...
    public PinchImageView(Context context, boolean initAlpha) {
        super(context);
    ...
    private boolean initAlpha = true;

    private void initAlphaDrawable() {
        this.alphaDrawable = new AlphaDrawable((int) (getContext().getResources().getDisplayMetrics().density * 12.0f), initAlpha);
        this.alphaDrawable.setBounds(
            Math.round(this.rect.left), 
            Math.round(this.rect.top), 
            Math.round(this.rect.right), 
            Math.round(this.rect.bottom)
        );
        invalidate();
    }
